...
const MAX_RETRIES = 6;
const FAILURE_FILE = path.join(__dirname, 'active-credit-order-gsi3-backfill-failures.json');
...
function getExpectedKeys(item) {
  ...
  return {
    GSI3PK: `BAR#${item.barId}#ACTIVE-CREDIT`,
    GSI3SK: `${customerId}#${item.createdAt}#${orderId}`
  };
}
...
function hasValidKeys(item, expectedKeys) {
  return Boolean(
    expectedKeys
    && item.GSI3PK === expectedKeys.GSI3PK
    && item.GSI3SK === expectedKeys.GSI3SK
  );
}
...
async function updateActiveCreditOrderGsi(item, expectedKeys) {
  const customerId = getCustomerId(item);
  const recordedBalance = Number(item.balanceDue);
  const fallbackBalance = Number(item.totalAmount || 0) - Number(item.amountPaid || 0);
  const balanceDue = Number.isFinite(recordedBalance) ? recordedBalance : fallbackBalance;
  ...
  return sendWithRetry(() => new UpdateCommand({
    ...
    ExpressionAttributeValues: {
      ':gsiPk': expectedKeys.GSI3PK,
      ':gsiSk': expectedKeys.GSI3SK,
      ':barId': item.barId,
      ':customer': customerId,
...
async function backfill() {
  ...
  do {
    ...
    if (APPLY) {
      await updateWithConcurrency(pendingUpdates, async ({ item, expectedKeys }) => {
        try {
          await updateActiveCreditOrderGsi(item, expectedKeys);
          counts.updated += 1;
        } catch (error) {
  ...
  console.log('\nActive credit GSI3 backfill summary');
...
backfill().catch((error) => {
  console.error('Active credit GSI3 backfill stopped:', error);
  process.exitCode = 1;
});
